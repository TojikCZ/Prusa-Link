...
KERNEL_URL = "http://security.debian.org/debian-security/pool/updates/main/" \
             "l/linux/linux-image-5.10.0-23-armmp-lpae_5.10.179-1_armhf.deb"
match = KERNEL_REGEX.match(KERNEL_URL)
...
DATA_PATH = str(files("prusa.link.data").joinpath("image_builder"))
...
def insert_from_file_before_line(to_file, from_file, search=None, index=None):
    ...
    result = lines[:index] + lines_to_insert + lines[index:]
...
def basic_image_setup():
    ...
    with open(userconf_path, "w") as userconf:
        userconf.write(
    ...
    with open(ssh_file_path, "w") as _:
        ...
...
def build_image():
    ...
    boot_message_path = join(DATA_PATH, "boot-message.service")
    ...
    with open(config_txt_path, "a") as config_txt:
    ...
    insert_from_file_before_line(
        ...
        from_file=join(DATA_PATH, "first-boot.sh"),
        index=1)
    ...
    if args.multi_instance:
        ...
        insert_from_file_before_line(
            ...
            from_file=join(DATA_PATH, "manager-start-script.sh"),
            search="exit 0")
    else:
        rc_local_bak_path = join(ROOTFS_MOUNT, "etc/rc.local.bak")
        insert_from_file_before_line(
            to_file=rc_local_bak_path,
            from_file=join(DATA_PATH, "prusalink-start-script.sh"),
            search="exit 0")
